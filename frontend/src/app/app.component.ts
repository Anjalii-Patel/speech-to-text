...
export class AppComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    from(this.wsService.generateSummary(currentTranscription))
      ...
      .subscribe({
        next: (data: any) => {
          ...
          // Only trigger if there is an uploaded file and transcription
          if (this.selectedFile && currentTranscription && currentTranscription.trim() !== '') {
  ...
  // Toggle Beam Size panel visibility
  toggleBeamSize(): void {
    this.showBeamSizePanel = !this.showBeamSizePanel;

    if (!this.showBeamSizePanel) {
      this.beamSize = 2; // reset if needed
    } else {
      setTimeout(() => {
        const beamSizeContainer = document.querySelector('.beam-size-container');
        if (beamSizeContainer) {
          beamSizeContainer.classList.add('visible');
          this.scrollToElement('.beam-size-container');
        }
      }, 100);
    }

    setTimeout(() => {
      document.querySelectorAll('.beam-size-container').forEach((container) => {
        if (this.showBeamSizePanel) {
          container.classList.add('visible');
        } else {
          container.classList.remove('visible');
    ...
    console.log(
      'Beam Size panel toggled:',
      this.showBeamSizePanel ? 'visible' : 'hidden'
    );
  }
  ...
  askLlama(): void {
    ...
    this.wsService
      ...
      .catch((err: Error) => {
        console.error('WebSocket error:', err);
        this.llamaResponse = `Error: ${err.message || 'Error communicating with AI service'
          }`;
        this.processingTranscription = false;
        this.showToast(
          'error',
  ...
  // Reload Whisper model with new beam size
  reloadWhisper(): void {
    console.log(`Reloading Whisper with beam size: ${this.beamSize}`);
  
    const formData = new FormData();
    formData.append('new_beam_size', String(this.beamSize));
  
    const reloadUrl = `${this.wsService.getHttpBaseUrl()}/update-beam-size/`;
    ...
    fetch(reloadUrl, {
      ...
      .then(async (res) => {
        ...
        this.showToast(
          'success',
          `Beam size updated to ${confirmedBeamSize}.`,
          'Model Reloaded.',
          3000
        );
        // alert(`Beam size updated to ${this.beamSize} and model reloaded.`);
        this.toggleBeamSize(); // close panel
      })
      .catch((err) => {
        console.error('Error reloading Whisper:', err);
        alert('Failed to reload Whisper model. Please try again.');
      });
  }  
  ...
  async toggleRecording(): Promise<void> {
    ...
    if (!this.isRecording) {
      ...
      try {
        ...
        this.transcription = `Microphone access error: ${err instanceof Error ? err.message : 'Could not access microphone'
          }`;
        this.showToast(
          'error',
          'Microphone Error',
  ...
  private createAudioFromChunks(): void {
    try {
      ...
      // Assign the audio Blob as a File to selectedFile for upload/saving
      this.selectedFile = new File([blob], 'recording.wav', { type: 'audio/wav' });
      this.selectedFileName = 'recording.wav';
    } catch (error) {
